...
class EmailFilterTasks:
    ...
    def filter_emails_tasks(self, agent, emails):
        return Task(
            description=dedent(f"""\
				...
				Your final answer MUST be for the relevant thread_ids and the sender, use bullet points of only the
				important emails worth needing attention.
				"""),
			agent=agent
		)
    ...
    def action_required_email_task(self, agent):
        return Task(
            description=dedent("""\
        				For each email thread, pull and analyze the complete threads using the actual Thread ID.
        				...
        				Your final answer MUST be a list for all emails with each consisting:
        				- the thread_id
